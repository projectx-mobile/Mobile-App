                ...
                .map((x) -> {
                    ...
                    return (ChildDto.builder()
                            ...
                            .numberOfCompletedTasks(x.getTasks().stream()
                                    .filter(familyTask -> familyTask.getTaskStatus() == TASK_STATUS.COMPLETED)
                                    .count())
                            .numberOfActiveTasks(x.getTasks().stream()
                                    .filter(familyTask -> familyTask.getTaskStatus() == TASK_STATUS.ACTIVE)
                                    .count())
                            ...
                            .taskRequestIdsDtos(x.getTasks().stream()
                                    .filter(task -> task.getTaskStatus() == TASK_STATUS.PENDING)
